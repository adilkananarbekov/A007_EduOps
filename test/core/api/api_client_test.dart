...
import 'package:eduprog/core/api/api_constants.dart';
import 'package:eduprog/core/api/api_exception.dart';
...
void main() {
  group('ApiClient', () {
    ...
    test('keeps auth requests on the remote backend when login fails', () async {
      ApiConstants.setRuntimeBaseUrlOverride(ApiConstants.cloudBackendUrl);
      ...
      await expectLater(
        ...
        throwsA(
          isA<NetworkException>().having(
            (error) => error.message,
            'message',
            contains(ApiConstants.cloudBackendUrl),
          ),
        ),
      );

      expect(remoteAttempts, 1);
      expect(ApiConstants.baseUrl, ApiConstants.cloudBackendUrl);
    });

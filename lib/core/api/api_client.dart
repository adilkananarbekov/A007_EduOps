...
class ApiClient {
  ...
  String _networkFailureMessage([Object? error]) {
    ...
    final isRemoteBackend = baseUrl == ApiConstants.cloudBackendUrl;
    ...
    if (isRemoteBackend) {
      if (closedBeforeHeaders) {
        return 'The remote backend at ${ApiConstants.cloudBackendUrl} '
            'closed the connection before finishing the response.';
      }

      return 'Unable to reach the remote backend at ${ApiConstants.cloudBackendUrl}.';
    }
  ...
  ApiException _mapAuthenticationException({
    ...
    if (includeAuth || endpoint != ApiConstants.login) {
      return error;
    }

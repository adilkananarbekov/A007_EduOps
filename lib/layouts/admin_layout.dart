...
Future<bool> _shouldAllowRouteChange(
  BuildContext context,
  WidgetRef ref,
  String currentRoute,
  String targetRoute,
) async {
  final currentPath = Uri.parse(currentRoute).path;
  final targetPath = Uri.parse(targetRoute).path;

  if (currentPath == targetPath) {
    return false;
  }

  final leaveGuard = ref.read(adminRouteLeaveGuardProvider);
  if (leaveGuard == null) {
    return true;
  }

  return leaveGuard(targetPath);
}

Future<void> _goToAdminRoute(
  BuildContext context,
  WidgetRef ref,
  String currentRoute,
  String targetRoute,
) async {
  final shouldNavigate = await _shouldAllowRouteChange(
    context,
    ref,
    currentRoute,
    targetRoute,
...
Future<void> _handleAdminBack(
  ...
  await _goToAdminRoute(context, ref, currentRoute, fallbackRoute);
}
...
class _Sidebar extends ConsumerWidget {
  ...
  Future<void> _handleLogout(BuildContext context, WidgetRef ref) async {
    final shouldLogout = await _shouldAllowRouteChange(
      context,
      ref,
      currentRoute,
      '/login',
...
class _SidebarNavItem extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Padding(
      ...
      child: InkWell(
        onTap: () => _goToAdminRoute(context, ref, currentRoute, item.route),
        borderRadius: BorderRadius.circular(AppSpacing.radiusMd),
        child: Container(
            ...
            : SafeArea(
                ...
                child: Container(
                  ...
                  child: BottomNavigationBar(
                    ...
                    onTap: (index) => _goToAdminRoute(
                      context,
                      ref,
                      currentRoute,
                      visibleBottomNavItems[index].route,
                    ),
